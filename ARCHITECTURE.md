...
omagenda/
  ...
  omagenda/                # Python package imported by bin/omagenda (sys.path insert)
    ...
    sync.py                # pimsync / vdirsyncer / omacal / ICS subscriptions
...
No build step. Installation is `omarchy plugin add <git-url>` plus `omarchy pkg add python-icalendar python-dateutil python-recurring-ical-events` (and optionally `pimsync` or `vdirsyncer`). `omagenda doctor` verifies all of it.
...
- **Calendar-side config** the CLI needs without the shell running lives in `~/.config/omagenda/config.toml` (read with `tomllib`):

```toml
vdir = "~/.local/share/calendars"
sync = "pimsync"            # pimsync | vdirsyncer | none | omacal
default_calendar = "personal"

[sets]
work = ["work", "team-holidays"]
home = ["personal", "family"]

[[subscriptions]]
name = "Canada Holidays"
url = "https://…/holidays.ics"
color = "yellow"
